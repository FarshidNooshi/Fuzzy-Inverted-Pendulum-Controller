...
def do_fuzzify_and_inference(inputs):
    ...
    return methods.calculate_with_rules_4_vars(inputs)
...
class FuzzificationMethods:
    ...
    def left_far_cp(self, x):
        points = ((-10, 0), (-10, 1), (-5, 0))
        return get_fuzzify(x, points, is_left_free=True)

    def left_near_cp(self, x):
        points = ((-10, 0), (-2.5, 1), (0, 0))
        return get_fuzzify(x, points)

    def stop_cp(self, x):
        points = ((-2.5, 0), (0, 1), (2.5, 0))
        return get_fuzzify(x, points)

    def right_near_cp(self, x):
        points = ((0, 0), (2.5, 1), (10, 0))
        return get_fuzzify(x, points)

    def right_far_cp(self, x):
        points = ((5, 0), (10, 1), (10, 0))
        return get_fuzzify(x, points, is_right_free=True)

    # cv
    def left_fast_cv(self, x):
        points = ((-5, 0), (-5, 1), (-2.5, 0))
        return get_fuzzify(x, points, is_left_free=True)
    ...
    def calculate_with_rules_4_vars(self, inputs):
        ...
        cv_list = ["stop", "left_fast", "left_slow", "right_slow", "right_fast"]
        cp_list = ["left_near", "stop", "right_far", "right_near", "left_far"]
        pa_values = dict()
        pv_values = dict()
        cp_values = dict()
        cv_values = dict()
        ...
        for item in cv_list:
            cv_values[item] = getattr(methods, item + '_cv')(inputs['cv'])
        for item in cp_list:
            cp_values[item] = getattr(methods, item + '_cp')(inputs['cp'])

        return self.apply_rules(pa_values=pa_values, pv_values=pv_values, cv_values=cv_values, cp_values=cp_values)

    def apply_rules(self, pa_values, pv_values, cv_values, cp_values):
        results = dict()
        ...
        rules = [('up_more_right', 'ccw_slow', 'left_fast', 'stop'),
                 ('up_more_right', 'cw_slow', 'left_fast', 'stop')]
        for item_pa, item_pv, item_cv, item_cp in rules:
            results['right_fast'] = max(results['right_fast'],
                                        min(pa_values[item_pa], pv_values[item_pv], cv_values[item_cv],
                                            cp_values[item_cp]))

        rules = [('up_more_left', 'ccw_slow', 'right_fast', 'stop'),
                 ('up_more_left', 'cw_slow', 'right_fast', 'stop')]
        for item_pa, item_pv, item_cv, item_cp in rules:
            results['left_fast'] = max(results['left_fast'],
                                        min(pa_values[item_pa], pv_values[item_pv], cv_values[item_cv],
                                            cp_values[item_cp]))
        return results
